...
generate_object_object (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  json_object_set_double_member (objectObj, "orientation", 0);
	...
	cout<<NVDS_OBJECT_TYPE_PERSON<<NVDS_OBJECT_TYPE_FACE<<NVDS_OBJECT_TYPE_PERSON_EXT<<NVDS_OBJECT_TYPE_FACE_EXT<<endl;
...
static gchar*
generate_deeppose_message (NvDsMsg2pCtx *ctx, NvDsEvent *events, guint size)
{
  ...
  JsonNode *rootNode;
  JsonObject *jobject,*jLandmarks; 
  JsonArray  *jArray, *jArrayKeypoints;
  ...
  jLandmarks = json_object_new ();
  jArrayKeypoints = json_array_new ();
    ...
    if (meta->extMsg && meta->extMsgSize) {
      ...
      switch (meta->objType) {
        case NVDS_OBJECT_TYPE_FACE: {
          ...
          break;
	case TYPE_FACIAL_KEYPOINTS: {
	  FacialKeyPointsCoords *dsObj = (FacialKeyPointsCoords *) meta->extMsg;
          if (dsObj) {
            for(unsigned int i=0; i<NUM_FACIAL_KEYPOINTS; i++) {
              json_array_add_double_element(jArrayKeypoints, dsObj->keypoints[i].x);
              json_array_add_double_element(jArrayKeypoints, dsObj->keypoints[i].y);
            }
          }
	  json_object_set_string_member(jLandmarks, "type", "facial");
	  json_object_set_array_member (jLandmarks, "kps", jArrayKeypoints);
	}
	  break;
	case TYPE_BODY_KEYPOINTS: {
	  BodyKeyPointsCoords *dsObj = (BodyKeyPointsCoords *) meta->extMsg;
          if (dsObj) {
	    for(unsigned int i=0; i<NUM_BODY_KEYPOINTS; i++) {
	      json_array_add_double_element(jArrayKeypoints, dsObj->keypoints[i].x);
	      json_array_add_double_element(jArrayKeypoints, dsObj->keypoints[i].y);
	      json_array_add_double_element(jArrayKeypoints, dsObj->keypoints[i].z);
	    }
	  }
	  json_object_set_string_member(jLandmarks, "type", "body");
	  json_object_set_array_member (jLandmarks, "kps", jArrayKeypoints);
	}
	  break;
        default:
  ...
  //json_object_set_array_member (jobject, "kps", jArrayKeypoints);
  json_object_set_object_member(jobject, "landmarks", jLandmarks);
                     ...
                     guint *payloadCount)
{
  ...
  *payloadCount = 0;
  ...
  if (ctx->payloadType == NVDS_PAYLOAD_CUSTOM) {
    message = generate_deeppose_message (ctx, events, eventSize);
    if (message) {
...
nvds_msg2p_generate (NvDsMsg2pCtx *ctx, NvDsEvent *events, guint size)
{
  ...
  if (ctx->payloadType == NVDS_PAYLOAD_CUSTOM) {
    message = generate_deeppose_message (ctx, events, size);
    if (message) {
      len = strlen (message);
